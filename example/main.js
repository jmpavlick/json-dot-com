...
function _VirtualDom_render(vNode, eventNode)
{
	...
	if (tag === 4)
	{
		return _VirtualDom_render(vNode.k, function (msg) { return eventNode(vNode.j(msg)) });
	}
...
function _VirtualDom_renderTranslated(vNode, eventNode)
{
	...
	if (tag === 5)
	{
		return _VirtualDom_renderTranslated(vNode.k, eventNode);
	}

	if (tag === 4)
	{
		return _VirtualDom_renderTranslated(vNode.k, function (msg) { return eventNode(vNode.j(msg)) });
	}

	var newDomNodes = vNode[_VirtualDom_instance].s;
...
function _VirtualDom_storeDomNode(vNode, domNode)
{
	...
	if (vNode_.t !== _VirtualDom_renderCount)
	{
		vNode_.u = vNode_.s;
		vNode_.s = [];
		vNode_.v = 0;
		vNode_.t = _VirtualDom_renderCount;
	}
	vNode_.s.push(domNode);
}
...
function _VirtualDom_diffHelp(x, y, eventNode)
{
	...
	{
		return {
			w: _VirtualDom_quickVisit(x, y, eventNode),
			x: false,
			y: false
		};
	}
	...
	if (y.$ === 4)
	{
		return _VirtualDom_diffHelp(x, y.k, function (msg) { return eventNode(y.j(msg)) });
	}
	...
	if (x.$ === 5)
	{
		if (y.$ === 5)
		{
			...
			y.k = y.m();
			return _VirtualDom_diffHelp(x.k, y.k, eventNode);
		}
		else
		{
			return _VirtualDom_diffHelp(x.k, y, eventNode);
		}
	}

	if (y.$ === 5)
	{
		return _VirtualDom_diffHelp(x, y.k || (y.k = y.m()), eventNode);
	}
		...
		case 0:
			if (x.a !== y.a)
			{
				...
				if (_VirtualDom_everTranslated)
				{
					...
					y[_VirtualDom_instance].s[y[_VirtualDom_instance].s.length - 1] = newNode;
...
function _VirtualDom_quickVisit(x, y, eventNode)
{
	...
	{
		case 4:
			return _VirtualDom_quickVisit(x.k, y.k, function (msg) { return eventNode(y.j(msg)) });

		case 5:
			return _VirtualDom_quickVisit(x.k, y.k, eventNode);
	}
...
function _VirtualDom_removeVisit(x, shouldRemoveFromDom)
{
	...
	{
		case 4:
			_VirtualDom_removeVisit(x.k, shouldRemoveFromDom);
			return;
	...
	if (x_.t === _VirtualDom_renderCount)
	{
		domNode = x_.u[x_.v];
		x_.v++;
	...
	{
		domNode = x_.s[0]; // Read from the to-be `oldDomNodes` (see below).
	...
	switch (x.$)
	{
		...
		case 1:
			for (var kids = x.e, i = 0; i < kids.length; i++)
			{
...
// Consume DOM node number `v` from `x`:s "old" nodes,
...
function _VirtualDom_consumeDomNode(x, y)
{
	...
	if (y_.t !== _VirtualDom_renderCount)
	{
		y_.u = y_.s;
		y_.s = [];
		y_.v = 0;
		y_.t = _VirtualDom_renderCount;
	}
	if (x_.t === _VirtualDom_renderCount)
	{
		var domNode = x_.u[x_.v];
		y_.s.push(domNode);
		x_.v++;
		return domNode;
	}
...
function _VirtualDom_diffKeyedKids(parentDomNode, xParent, yParent, eventNode)
{
	...
	var xKidsMap = xParent.r;
	var yKidsMap = yParent.r;
	...
	while (true)
	{
		...
		if (xIndexLower < xIndexUpper && yIndexLower < yIndexUpper)
		{
			...
			if (xKeyLower === yKeyUpper)
			{
				...
				xIndexLower++;
				yIndexUpper--;
				_VirtualDom_moveBefore(parentDomNode, diffReturn.w, domNodeUpper);
				handleDiffReturn(diffReturn, true);
				swapped = true;
			}
			...
			if (xKeyUpper == yKeyLower)
			{
				...
				yIndexLower++;
				xIndexUpper--;
				_VirtualDom_moveAfter(parentDomNode, diffReturn.w, domNodeLower);
				handleDiffReturn(diffReturn, false);
				swapped = true;
			}
		}
...
function _VirtualDom_wrap(object)
{
	...
	Object.defineProperty(object, _VirtualDom_instance, {
		value: {
			// We only read from `x.u`. Uses `v`. Is set to `s` at each render.
			u: [],
			// This is set to a new, empty array on each render. We push to `y.s`. The reason we have to have two arrays is because the same virtual node can be used multiple times, so sometimes `x === y`.
			s: [],
			t: 0,
			// The index of the next DOM node in `u` to use.
			v: 0
		}
	});
}
...
function _VirtualDom_applyPatches(rootDomNode, oldVirtualNode, newVirtualNode, eventNode)
{
	...
	// We can’t do anything about `diffReturn.x` or
	// `diffReturn.y` here, because we don’t know the parent of the
...
function _VirtualDom_applyPatchRedraw(x, y, eventNode)
{
	...
	var domNode = y[_VirtualDom_instance].s.pop();
	var parentNode = domNode.parentNode;
...
function _VirtualDom_virtualizeHelp(node)
{
	...
	if (node.nodeType === 3)
	{
		...
		_VirtualDom_wrap(vNode);
		vNode[_VirtualDom_instance].s.push(node);
		return vNode;
	}
	...
	else
	{
		for (var kids = node.childNodes, i = kids.length; i--; )
		{
			...
			if (kidNode)
			{
				kidList = _List_Cons(kidNode, kidList);
			}
		}
...
var $elm$browser$Debugger$Main$initialWindowHeight = 1600;
...
var $elm$browser$Debugger$Expando$maximumItemsToView = F2(
	function (path, expando) {
		return A2(
			...
			1,
			A2($elm$core$Dict$get, path, expando.viewMore)) * 100;
	});
...
var $elm$browser$Debugger$Expando$elideMiddle = $elm$core$Basics$identity;
		...
		while (true) {
			if (_Utils_cmp(index, max) < 0) {
				if (!keyValuePairs.b) {
					...
					var $temp$path = path,
						$temp$expando = expando,
						$temp$index = index + 1,
						$temp$max = max,
						$temp$keyValuePairs = rest,
						$temp$acc = A2(
						$elm$core$List$cons,
						A3(
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					A2($elm$html$Html$Attributes$style, 'background-color', 'rgb(27, 123, 110)'),
					A2($elm$html$Html$Attributes$style, 'pointer-events', block),
					A2($elm$html$Html$Attributes$style, 'user-select', block)
				]),
			_List_fromArray(
		...
		return _Utils_Tuple2(
			{
				...
				layout: A3($elm$browser$Debugger$Main$Vertical, $elm$browser$Debugger$Main$Static, 0.3, 0.5),
...
var $author$project$Main$init = F3(
	function (_v0, url, key) {
		return _Utils_Tuple2(
			{key: key, url: url, xorSet: $elm$core$Set$empty},
			$elm$core$Platform$Cmd$none);
	});
...
var $elmcraft$core_extra$List$Extra$Continue = function (a) {
	return {$: 'Continue', a: a};
};
var $elmcraft$core_extra$List$Extra$Stop = function (a) {
	return {$: 'Stop', a: a};
};
var $elmcraft$core_extra$List$Extra$stoppableFoldl = F3(
	function (func, acc, list) {
		stoppableFoldl:
		while (true) {
			if (!list.b) {
...
var $author$project$Json$DotCom$batch = F3(
	function (_v0, matchers, bUrlRequest) {
		var onBrowserInternal = _v0.onBrowserInternal;
		var onBrowserExternal = _v0.onBrowserExternal;
		var matchResult = A3(
			$elmcraft$core_extra$List$Extra$stoppableFoldl,
			F2(
				function (stepMatcher, acc) {
					var _v3 = stepMatcher(bUrlRequest);
					if (_v3.$ === 'Ok') {
						var parsed = _v3.a;
						return $elmcraft$core_extra$List$Extra$Stop(
							$elm$core$Result$Ok(parsed));
					} else {
						return $elmcraft$core_extra$List$Extra$Continue(acc);
					}
				}),
			$elm$core$Result$Err(bUrlRequest),
			matchers);
		if (matchResult.$ === 'Err') {
			var bur = matchResult.a;
			if (bur.$ === 'Internal') {
				var url = bur.a;
				return onBrowserInternal(url);
			} else {
				var str = bur.a;
				return onBrowserExternal(str);
			}
		} else {
			var parsed = matchResult.a;
			return parsed;
		}
	});
...
var $author$project$Json$DotCom$token = 'https://json.com/';
...
var $author$project$Json$DotCom$parseHref = function (hrefStr) {
...
var $author$project$Main$update = F2(
	function (msg, model) {
		if (msg.$ === 'OnUrlRequest') {
			...
			return A3(
				$author$project$Json$DotCom$batch,
				{
					onBrowserExternal: function (str) {
						return _Utils_Tuple2(
							model,
							$elm$browser$Browser$Navigation$load(str));
					},
					onBrowserInternal: function (url) {
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{url: url}),
				...
				_List_fromArray(
					[
						$author$project$Modal$onUrlRequest(model)
					]),
				urlRequest);
		} else {
			var url = msg.a;
	...
	return {
		body: _List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text('elm is a pl that i like')
					])),
				A2(
				$elm$html$Html$a,
				_List_fromArray(
					[
						$elm$html$Html$Attributes$href('#warble')
					]),
				_List_fromArray(
					[
						$elm$html$Html$text('click the warble')
					])),
				A2(
				$elm$html$Html$div,
				_List_Nil,
				_List_fromArray(
					[
						$elm$html$Html$text(
						$elm$core$Debug$toString(model.url))
					])),
				A2(
				$elm$html$Html$div,
				_List_Nil,
...
_Platform_export({'Main':{'init':$author$project$Main$main(
	$elm$json$Json$Decode$succeed(_Utils_Tuple0))({"versions":{"elm":"0.19.1"},"types":{"message":"Main.Msg","aliases":{"Url.Url":{"args":[],"type":"{ protocol : Url.Protocol, host : String.String, port_ : Maybe.Maybe Basics.Int, path : String.String, query : Maybe.Maybe String.String, fragment : Maybe.Maybe String.String }"}},"unions":{"Main.Msg":{"args":[],"tags":{"OnUrlRequest":["Browser.UrlRequest"],"OnUrlChange":["Url.Url"]}},"Basics.Int":{"args":[],"tags":{"Int":[]}},"Maybe.Maybe":{"args":["a"],"tags":{"Just":["a"],"Nothing":[]}},"Url.Protocol":{"args":[],"tags":{"Http":[],"Https":[]}},"String.String":{"args":[],"tags":{"String":[]}},"Browser.UrlRequest":{"args":[],"tags":{"Internal":["Url.Url"],"External":["String.String"]}}}}})}});
